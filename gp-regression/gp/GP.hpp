...
{
public:
	inline GP(unsigned int pop_size, unsigned int num_iter, unsigned int max_height, unsigned int tour_size, unsigned int seed, double mut_rate, 
		  short mode_init,  const std::vector<std::string>& terminals, const std::vector<std::string>& operators) : pop_size(pop_size), num_iter(num_iter), 
		  max_height(max_height), tour_size(tour_size), seed(seed), mut_rate(mut_rate), mode_init(mode_init), terminals(terminals), operators(operators),
		  generator(seed)
	{
	...
	inline virtual std::string run(const Dataset& train, const Dataset& test)
	{
		...
		std::pair<unsigned int, double> best_at_train = cur_pop->best(train, 0);
...
protected:
	unsigned int pop_size, num_iter, max_height, tour_size, seed;
	double mut_rate;
	short mode_init;
