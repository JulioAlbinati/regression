...
void GP::initialize_pop(Digraph& graph)
{
	...
	for (unsigned int i = 0; i < num_full; ++i)
		cur_pop->add_solution(Individual::build_random_individual(max_height, true, terminals, operators, graph));
	for (unsigned int i = 0; i < num_grow; ++i)
		cur_pop->add_solution(Individual::build_random_individual(max_height, false, terminals, operators, graph));
}
...
Individual* GP::mutation(Individual* original, Digraph& digraph)
{
	...
	if (coin < 0.5)
	{
		unsigned int height = rand() % max_height + 1;
		Individual* subtree = Individual::build_random_individual(height, false, terminals, operators, digraph);
		unsigned int chosen = rand() % digraph.size(original->index());
		...
		unsigned int pruned_height = rand() % max_height + 1;
		unsigned int pruned_index = Individual::prune(full->index(), pruned_height, digraph, terminals, values, count);
	...
	{
		Individual* subtree = Individual::build_random_individual(1, false, terminals, operators, digraph);
		unsigned int chosen = rand() % digraph.size(original->index());
		std::pair< unsigned int, std::vector<double> > modified = change_subexpression(original, subtree->index(), chosen, digraph, subtree->get_values());
		Individual* full = new Individual(modified.first, &digraph, modified.second);
		delete subtree;
...
{
	Individual* subtree = ind1->random_subexpression();
	unsigned int chosen = rand() % digraph.size(ind2->index());
