...
public:
	inline OptimizedCrossLinearSGP(unsigned int pop_size, unsigned int num_iter, unsigned int max_height, unsigned int mut_height, unsigned int tour_size, 
		  unsigned int seed, double mut_rate, double mut_step, short mode_init, unsigned int num_terminals, const std::vector<std::string>& operators) : 
		  OptimizedCrossSGP(pop_size, num_iter, max_height, mut_height, tour_size, seed, mut_rate, mut_step, mode_init, num_terminals, operators) {}
protected:
