...
Individual::Individual(const std::string& operation, const Individual& left, const Individual& right, DAG& graph) : num_elements(left.size() + right.size() + 1)
{
	...
	index = graph.add_intermediate_node(operation, left.get_index(), right.get_index());
}
	...
	const std::vector<std::string>& operators, const Dataset& train, const Dataset& test, DAG& graph)
{
	...
	for (int i = tree_matrix.size() - 1; i >= 0; --i)
	{
		...
		for (unsigned int j = 0; j < tree_matrix[i].size(); ++j)
		{
			...
			{
				new_roots.push_back(Individual(operators[tree_matrix[i][j].first], roots[cur_index], roots[cur_index + 1], graph));
				cur_index += 2;
			}
			else if (tree_matrix[i][j].second == 1)
				new_roots.push_back(Individual(tree_matrix[i][j].first, train, test, graph));
			else
			{
				double value = ((double) rand()) / (RAND_MAX / 2) - 1.0;
				new_roots.push_back(Individual(value, train, test, graph));
			}
		}
		roots = new_roots;
	}
