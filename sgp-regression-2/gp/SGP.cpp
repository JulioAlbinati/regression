...
void SGP::initialize_pop(const Dataset& train, const Dataset& test)
{
	...
	for (unsigned int i = 0; i < num_full; ++i)
		cur_pop->add_solution(new Individual(Individual::build_random_individual(max_height, true, num_terminals, operators, train, test, *graph)));
	for (unsigned int i = 0; i < num_grow; ++i)
		cur_pop->add_solution(new Individual(Individual::build_random_individual(max_height, false, num_terminals, operators, train, test, *graph)));
}
...
{
	Individual random_ind1 = Individual::build_random_individual(mut_height, false, num_terminals, operators, train, test, *graph);
        Individual random_ind2 = Individual::build_random_individual(mut_height, false, num_terminals, operators, train, test, *graph);
	random_ind1.apply_sigmoid(*graph);
	random_ind2.apply_sigmoid(*graph);
        Individual temp1("-", random_ind1, random_ind2, *graph);
	Individual ms(mut_step, train, test, *graph);
	Individual temp2("*", ms, temp1, *graph);
	return new Individual("+", temp2, original, *graph);
}
...
Individual* SGP::crossover(const Individual& ind1, const Individual& ind2, const Dataset& train, const Dataset& test)
{
	...
	Individual ind_coeff(coeff, train, test, *graph);
	Individual ind_compl(1.0 - coeff, train, test, *graph);
	Individual temp1("*", ind1, ind_coeff, *graph);
	Individual temp2("*", ind2, ind_compl, *graph);
	return new Individual("+", temp1, temp2, *graph);
}
